...
import { resolveColor } from './modules/colorResolution.js.php';
...
onmessage = event => {
  ...
  switch (instruction) {
    case 'resolve-color': returnData = computeEverything(data); break;
    case 'make-range-gradients': returnData = makeRangeGradients(data); break;
  }
...
/**  */
function computeEverything({ colorString, formatsData }) {
  ...
  if (interfaceColor instanceof Couleur) {
    [response.formatsData, response.name] = makeColorData(formatsData, interfaceColor);
    [response.css, response.metaLight, response.metaDark, response.hue, response.saturation] = makeCSS(interfaceColor);
  }
...
/**  */
function makeColorData(formatsData, userColor) {
  // Populates results in all formats
  let name;
  for (const format of formatsData) {
    if (format.prop.slice(0, 5) === 'color')
      format.value = userColor.expr(format.prop, { precision: 4 });
    else {
      format.value = userColor[format.prop];
...
function makeCSS(userColor) {
  ...
  return [
    ...
    metaLight,
    metaDark,
    Math.round(userColor.h),
    Math.round(userColor.s * 100) + '%'
  ];
}
...
/**  */
function makeRangeGradients({ rangeData, couleur, visibleFormat }) {
